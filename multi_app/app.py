...
class App(Application):
    ...
    def compose(self):
        ...
        dicom_rgb_mask_writer_spleen = DICOMRGBMaskWriterOperator(
            self,
            segment_descriptions=seg_descriptions_spleen,
            custom_tags=custom_tags_spleen,
            output_folder=app_output_path,
            name="dicom_rgb_mask_writer_spleen",
        )
        ...
        dicom_rgb_mask_writer_pancreas = DICOMRGBMaskWriterOperator(
            self,
            segment_descriptions=seg_descriptions_pancreas,
            custom_tags=custom_tags_pancreas,
            output_folder=app_output_path,
            name="dicom_rgb_mask_writer_pancreas",
        )
        ...
        # Create DICOM RGB Mask  for one of the inference output
        # Note below the dicom_seg_writer requires two inputs, each coming from a upstream operator.
        """
        self.add_flow(
            series_selector_op, dicom_rgb_mask_writer_spleen, {("study_selected_series_list", "study_selected_series_list")}
        )
        self.add_flow(bundle_spleen_seg_op, dicom_rgb_mask_writer_spleen, {("pred", "seg_image")})
        """
        # Create DICOM RGB Mask for one of the inference output
        # Note below the dicom_seg_writer requires two inputs, each coming from a upstream operator.
        self.add_flow(
            series_selector_op,
            dicom_rgb_mask_writer_pancreas,
            {("study_selected_series_list", "study_selected_series_list")},
        )
        
        self.add_flow(bundle_pancreas_seg_op, dicom_rgb_mask_writer_pancreas, {("pred", "seg_image")})
